...
export function applySavedColors() {
    ...
    if (userId) {
        ...
        const savedPrimary =
            localStorage.getItem(`${userId}_color_primary`) || DEFAULT_PRIMARY;
        const savedSecondary =
            localStorage.getItem(`${userId}_color_secondary`) ||
            DEFAULT_SECONDARY;
...
export function setupColorPicker() {
    const root = document.documentElement;
    const colorPrimaryInput = document.getElementById(
        'color_primary'
    ) as HTMLInputElement | null;
    const colorSecondaryInput = document.getElementById(
        'color_secondary'
    ) as HTMLInputElement | null;
    const colorResetBtn = document.getElementById(
        'color_reset'
    ...
    const savedSecondary =
        localStorage.getItem(getUserKey('color_secondary')) ||
        DEFAULT_SECONDARY;
