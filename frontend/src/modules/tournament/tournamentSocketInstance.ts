 ...
 */
export function createTournamentSocket(token: string): TournamentWebSocketClient {
  if (!token) throw new Error("❌ No se puede crear WebSocket sin token válido"); // Translation i18n needed
  if (!instance) {
