...
export function setupAvatarUpload() {
    ...
    const updateAvatarBtn = document.getElementById(
        'updateAvatar'
    ...
    function handleFile(file: File) {
        ...
        (
            window as unknown as { selectedAvatarFile?: File }
        ).selectedAvatarFile = file;
    }

    // 💾 Subida al backend
    updateAvatarBtn!.addEventListener('click', async () => {
        const file = (window as unknown as { selectedAvatarFile?: File })
            .selectedAvatarFile;
        ...
        try {
            const response = await fetch(
                apiUrl(`/user-manager/upload-avatar`),
                {
                    method: 'POST',
                    body: formData,
                    headers: {
            ...
            if (response.ok) {
                ...
                const avatarSrc = apiUrl(
                    userResponse.user.avatar || '/images/avatar1.jpg'
                );
