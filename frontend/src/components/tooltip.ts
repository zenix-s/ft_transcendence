...
export class Tooltip {
  ...
  public init(): void {
    this.inputs.forEach((input) => {
      ...
      const isInput = input.tagName === "INPUT" || input.tagName === "TEXTAREA" || input.tagName === "SELECT";

      if (isInput) {
        // 🎯 Caso formulario → focus / blur
        input.addEventListener("focus", () => this.showTooltip(tooltip));
        input.addEventListener("blur", () => this.hideTooltip(tooltip));
      } else {
        // 🎯 Caso general → hover
        input.addEventListener("mouseenter", () => this.showTooltip(tooltip));
        input.addEventListener("mouseleave", () => this.hideTooltip(tooltip));
      }
    });
  }
