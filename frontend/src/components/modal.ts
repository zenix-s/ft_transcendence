...
export async function modal({
  ...
  activeTournament?: ActiveTournament,
    ...
    else if (type === "setReady")
    {
      ...
      const winnerName = winnerParticipant ? winnerParticipant.username : t("Unknown");
      console.log("Tournament Winner Name:", winnerName);
      const status = activeTournament.status;
      const statusText = status === "ongoing" ? t("inProgress") :
...
function buildTournamentTable(activeTournament: ActiveTournament | undefined): HTMLTableElement {
  ...
  sorted.forEach((p, i) => {
    ...
    else if (i === 1) row.classList.add("bg-gray-300", "dark:text-secondary");
    else if (i === 2) row.classList.add("bg-yellow-800", "dark:text-secondary");
    else row.classList.add("sm:bg-transparent", "text-primary", "sm:text-secondary", "dark:text-primary");
    row.innerHTML = `
