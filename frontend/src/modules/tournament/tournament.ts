...
export async function createTournament(): Promise<number> {
	return new Promise((resolve, reject) => {
		...
		if (!tournamentForm) {
			console.warn("Tournament creation form not found");
		...
		tournamentForm.addEventListener("submit", async (e) => {
			...
			const mode = formData.get("mode-radio") as string;
			...
			*/
			...
			try {
				const response = await fetch(apiUrl(`/tournaments/pong`), {
					...
					body: JSON.stringify({
						...
						matchSettings: {
							maxScore: points,
							maxGameTime: time,
							//mode: mode, // Cuando el endpoint lo permita
						},
					}),
				});
				...
				console.log("Creado un torneo con ID = " + tournamentId);
